...
public class EmployeeTerminationFileProcessor extends CsvHandler implements FileProcessor
{
	...
	public  ColumnPositionMappingStrategy setColumMapping() {
		...
		String[] columns = new String[] {"companyCode","description","status","employeeNumber","firstName","lastName","position","terminationDate"};
	...
	public <T> List<T> cleanUp(List<T> list) {
		...
		if (list != null && !list.isEmpty()) {
			...
			for (Object obj : list) {
				...
				if (StringUtils.isNumeric(bean.getEmployeeNumber())) {
					newList.add(bean);
				}
